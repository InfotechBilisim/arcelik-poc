...
public class Extent {
    ...
    public boolean equals(Extent ext) {
        if (Math.abs(minLatitude - ext.getMinLatitude()) > 0.00001)
            return false;
        if (Math.abs(minLongitude - ext.getMinLongitude()) > 0.00001)
            return false;
        if (Math.abs(maxLatitude - ext.getMaxLatitude()) > 0.00001)
            return false;
        if (Math.abs(maxLongitude - ext.getMaxLongitude()) > 0.00001)
            return false;
        return true;
    } // equals()
    ...
    public Extent extrapolate(int x1, int y1, int x2, int y2, int w, int h) {
        ...
        double minLat =
            minLatitude + (h - (y1 > y2 ? y1 : y2)) * mulLat; // Since in screen coordinates 0,0 is top-left.
    ...
    public Extent Union(Extent ext) {
        ...
        double maxLat = maxLatitude;
        double maxLon = maxLongitude;
        if (ext.maxLatitude > maxLat)
            maxLat = ext.maxLatitude;
        if (ext.minLatitude < minLat)
            minLat = ext.minLatitude;
        if (ext.maxLongitude > maxLon)
            maxLon = ext.maxLongitude;
        if (ext.minLongitude < minLon)
            minLon = ext.minLongitude;
        return (new Extent(minLat, minLon, maxLat, maxLon));
    } // Union()
